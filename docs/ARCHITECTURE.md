...
- 当前用户信息接口。
- 个人知识库创建、列表、详情、逻辑删除。
- 删除知识库前检查是否存在未删除文档，有文档时返回 `409 Conflict`，避免业务记录和 vectors 残留在不可解释状态。
...
- V1 不支持同一文档同时执行索引和删除：文档处于 `PENDING_INDEX`、`INDEXING` 时，删除接口返回 `409004 DOCUMENT_BUSY`。
- RabbitMQ 投递失败时保留任务 `PENDING`，由简单 PENDING 任务重投器继续投递，也可通过 `POST /api/v1/documents/{documentId}/index-retry` 手动重投。
- 索引失败后也可通过 `POST /api/v1/documents/{documentId}/index-retry` 人工重试，此时会将失败任务重新置为 `PENDING`。
- 长时间停留在 `INDEXING`、`DELETING` 的文档会被简单超时标记器写入失败态，便于人工重试。
...
3. 抢占成功后写 MinIO object；如果 MinIO 写入失败，事务回滚。
4. Java 写入 `indexing_task`。
5. 注册事务提交回调。
...
1. 根据 `documentId`、`indexingTaskId` 查询 MySQL。
...
6. Java 校验 `indexed_chunk_count == chunk_count`。
...
10. 如果旧 worker 在超时或人工重试后迟到返回，Java 通过 MySQL 条件更新避免覆盖新的任务状态。
...
3. 如果文档处于 `PENDING_INDEX`、`INDEXING`，Java 返回 `409004 DOCUMENT_BUSY`。
4. Java 将 MySQL `document.is_deleted` 更新为 1，并将 `index_status` 更新为 `DELETING`。
...
8. Java 将 `document.index_status` 更新为 `DELETED`。

当前没有分布式事务；如果 Qdrant 或 MinIO 清理失败，Java 将文档标记为 `DELETE_FAILED`，并保留 `is_deleted=1`。V1 不引入补偿任务表或 outbox，重试方式是再次调用删除接口。
