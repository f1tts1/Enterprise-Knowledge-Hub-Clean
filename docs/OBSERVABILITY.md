...
| 字段 | 含义 |
|---|---|
| `answer_status` | Python 内部生成状态，只允许 `ANSWERED` 或 `INSUFFICIENT_CONTEXT` |
| `cited_context_indexes` | 答案正文实际出现且通过范围校验的 `[片段 n]` 编号；不是全部候选 chunk |
| `no_answer_reason` | 模型按受控 sentinel 拒答时为固定原因；正常回答时为 `null` |
...
Java 对外进一步收敛成三个 `answerStatus`：

- `ANSWERED`：LLM 返回正文且至少包含一个有效引用；`citations` 只映射实际引用的候选 chunk。
- `NO_CONTEXT`：没有检索命中或没有可用文本，由 Java 短路，不调用 LLM。
- `INSUFFICIENT_CONTEXT`：启用的相关性阈值过滤掉全部上下文，或 LLM 返回受控 no-answer sentinel；`citations` 为空。

`retrievedChunks` 始终表示权限过滤后的检索候选，不能与“答案实际采用的 `citations`”混为一谈。缺失引用、越界引用、sentinel 与其它文本混用都按生成协议失败处理，不伪装成成功回答。
...
- RAG：outcome、userId、kbId、contextCount、retrieval/generation/total 耗时、provider/model 和可用 usage；outcome 只使用 `answered`、`no_context`、`insufficient_context`、`failed`。
...
| LLM 返回有效回答 | `CHAT` | 是 | `success=1`；记录 provider/model、latency 和可用 prompt/completion usage |
| LLM 返回受控 no-answer sentinel | `CHAT` | 是 | provider 调用和响应协议成功，因此 `success=1`；RAG outcome 为 `insufficient_context`，不是模型调用失败 |
| 已发起生成但配置、HTTP 或响应处理失败 | `CHAT` | 是 | `success=0`，错误只保存安全类型/脱敏摘要 |
| 无上下文或阈值过滤后无上下文，Java 直接返回 | — | 否 | 不创建伪造 CHAT 记录 |
...
| `ekb.rag.duration` | Timer | `operation=ask`, `outcome` | Java RAG 总耗时；outcome 为 `answered`、`no_context`、`insufficient_context`、`failed` |
| `ekb.rag.phase.duration` | Timer | `phase`, `outcome` | retrieval/generation 分阶段耗时；Java 短路时不伪造 generation 阶段 |
...
代码中有针对 requestId 归一化/传播、RabbitMQ 异步关联、`model_call_log` best-effort、索引/检索/RAG outcome、受控 no-answer、实际引用映射、观测旁路异常以及 Python middleware/pipeline 成功和失败阶段的确定性测试。统一入口为：

```bash
./scripts/verify.sh
```

测试与证据分层见 `docs/TESTING.md`。2026-07-17 已针对当前工作树实际执行统一入口：Java 62 个测试、Python 30 个测试及 shell 语法检查通过；Compose 静态解析通过。2026-07-15 的 Java 46/46、Python 11/11 仅是 P0-2 历史记录。实际 Prometheus 文本、跨进程日志、数据库行和 Compose 运行结果仍必须在本地依赖启动后用真实调用留证；静态解析不能证明镜像已构建或业务链路已启动。本文不提供虚构运行数据。
